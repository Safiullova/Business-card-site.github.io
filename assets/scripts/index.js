class feedbackSlider {
	 ...
	 * @param {String} prefix
	 */
	constructor(el, config = {}, prefix = 'feedback-slider__') {
		this.#state = {
			prefix: prefix,
			el: el,
	...
	static createInstances() {
		document
			.querySelectorAll('[data-slider="feedback-slider"]')
			.forEach((el) => {
	...
	#attachEvents() {
		this.#state.events = {
			...
			'feedback-slider__transition-start': [
				this.#state.elItems,
	...
	#move() {
		...
		this.#state.elItems.dispatchEvent(
			new CustomEvent('feedback-slider__transition-start', {
				bubbles: true,
			})
		);
	}
...
feedbackSlider.createInstances();
